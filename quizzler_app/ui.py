from tkinter import *
from quiz_brain import QuizBrain

THEME_COLOR = "#375362"


# Pascal and no _ for class name
class QuizInterface:
    # Specify the quiz_brain object's type
    def __init__(self, quiz_brain: QuizBrain):
        self.quiz = quiz_brain

        # Window with score
        self.window = Tk()
        self.window.title("Quizzler")
        self.window.config(padx=20, pady=20, bg=THEME_COLOR)
        self.label = Label(text="Score: 0")
        self.label.config(bg=THEME_COLOR, font=("Arial", 10, "bold"), fg="white")
        self.label.grid(row=0, column=1)

        # Canvas with quiz questions
        self.canvas = Canvas(width=300, height=250, bg="white", highlightthickness=0)
        self.canvas.grid(row=1, column=0, columnspan=2, pady=50)
        self.question_text = self.canvas.create_text(
            150, 
            125, 
            width=280, 
            text="", 
            font=("Arial", 20, "italic")
        )

        # Buttons
        # Self allows the property to be accessed anywhere in the class, so images don't need self.
        true_image = PhotoImage(file=r"images\true.png")
        self.true_button = Button(image=true_image, highlightthickness=0, borderwidth=0, command=self.check_true)
        self.true_button.grid(row=2, column=0)
        false_image = PhotoImage(file=r"images\false.png")
        self.false_button = Button(image=false_image, highlightthickness=0, borderwidth=0, command=self.check_false)
        self.false_button.grid(row=2, column=1)

        # Generates first question
        self.get_next_question()

        self.window.mainloop()
    
    def get_next_question(self):
        self.canvas.config(bg="white")
        self.label.config(text=f"Score: {self.quiz.score}")
        if self.quiz.still_has_questions():
            q_text = self.quiz.next_question()
            self.canvas.itemconfig(self.question_text, text=q_text)
        else:
            self.canvas.itemconfig(self.question_text, text="You've reached the end of the quiz!")
            self.true_button.config(state="disable")
            self.false_button.config(state="disable")
    
    def check_true(self):
        self.give_feedback(self.quiz.check_answer("True"))
    
    def check_false(self):
        self.give_feedback(self.quiz.check_answer("False"))
        
    def give_feedback(self, is_right: bool):
        # Need to type out == "True" for the if statements to actually be checked...
        if is_right == "True":
            self.canvas.config(bg="green")
        else:
            self.canvas.config(bg="red")
        self.window.after(1000, self.get_next_question)
        