from flask import Flask
from random import randint, choice

app = Flask(__name__)

COLORS = ["CornflowerBlue", "CadetBlue", "BlueViolet", "DarkCyan", "DarkRed", "IndianRed", "MediumPurple"]

def color_text(function):
    def wrapper(*args, **kwargs):
        random_color = choice(COLORS)
        return f"<h1 style='color:{random_color}'>" + function(*args, **kwargs) + "</h1>"
    return wrapper


@app.route('/')
def homepage():
    return "<h1>Guess a number between 0 and 9</h1>" \
        "<img src='https://media.giphy.com/media/3o7aCSPqXE5C6T8tBC/giphy.gif'>"

correct_number = randint(0, 9)


@app.route('/<int:number_guessed>')
@color_text
def check_number(number_guessed):
    if number_guessed > correct_number:
        return "Too high!" \
            "<br><img src='https://media.giphy.com/media/3o6ZtaO9BZHcOjmErm/giphy.gif'>"
    elif number_guessed < correct_number:
        return "Too low!" \
            "<br><img src='https://media.giphy.com/media/jD4DwBtqPXRXa/giphy.gif'>"
    else:
        return f"You guessed the correct number: {number_guessed}!" \
            "<br><img src='https://media.giphy.com/media/4T7e4DmcrP9du/giphy.gif'>"


if __name__ == "__main__":
    app.run(debug=True)